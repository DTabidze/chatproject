...
const AddNewContact = () => {
  const { loggedInUser, refreshUser } = useContext(UserContext);
  ...
  useEffect(() => {
    async function fetchAllUsers() {
      try {
        const response = await fetch("http://192.168.1.162:8080/users", {
          method: "GET",
          credentials: "include",
        });
  ...
  useEffect(() => {
    async function fetchMyContactList() {
      try {
        const response = await fetch("http://192.168.1.162:8080/contacts", {
          method: "GET",
          credentials: "include",
        });
  ...
  function handleAddContact(id) {
    ...
    fetch("http://192.168.1.162:8080/contacts", {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json", // Corrected headers property
      },
